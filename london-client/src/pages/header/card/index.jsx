...
const Card = () => {
  ...
  return (
    ...
    >
      ...
      {isHovered &&
        (cart.items.length > 0 ? (
          <div className='absolute right-0 w-96 p-4 mt-2 bg-black border-white border-[0.5px] shadow-lg z-20 rounded-md flex flex-col'>
            <ul className='overflow-y-auto max-h-[350px] flex-grow'>
              {cart.items.map((product) => (
                ...
                >
                  <a
                    href={`/product/${product.item.item_id}`}
              ...
              <p className='text-xl font-semibold'>
                ...
                {cart.items
                  .reduce(
                    (acc, curr) =>
                      acc +
                      parseFloat(curr.item.price) *
                        parseFloat(curr.item.quantity),
                    0
                  )
                  .toFixed(2)}
              </p>
          ...
          <div className='absolute right-0 w-56 text-center p-4 mt-2 bg-black border-white border-[0.5px] shadow-lg z-20 rounded-md'>
